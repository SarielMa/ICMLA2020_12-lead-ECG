...
def dZdX_L2norm_max(model, X, Y, alpha=1, reduction='mean'):
    ...
    return loss
#%%
def dZdX_NSR(model, X, Y, noise_norm, norm_type,
             alpha=0, eps=1e-8, log_NSR=False, detach_signal=False, reduction='mean'):
    # this shoud only be applied for correctly classified samples using MSE or MAE loss
    X=X.detach()
    X.requires_grad=True
    Z=model(X)
    if len(Z.size()) > 1:
    ...
    X_grad=X_grad.view(X_grad.size(0), -1)
    #---------------------
    Noise = noise_norm*torch.norm(X_grad, p=norm_type, dim=1)
    #---------------------
    if detach_signal == True:
        Signal = Signal.detach()          
    #---------------------
